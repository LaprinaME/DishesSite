...
{
    public class DeleteCommentsControllerTests
    {
        [Fact]
        public async Task Index_ReturnsViewResult_WithListOfComments()
        {
            ...
            {
                var controller = new DeleteCommentsController(context);
                ...
                var model = Assert.IsAssignableFrom<List<DeleteCommentsViewModel>>(viewResult.Model);
